...
def create_bee_image(bee_name, width=150, height=150):
    ...
    # Draw wings (semi-transparent effect with lines)
    wing_positions = [(width//4 - 20, height//2), (3*width//4 + 20, height//2)]
    for wing_x, wing_y in wing_positions:
        draw.ellipse([wing_x - 15, wing_y - 25, wing_x + 15, wing_y + 25], 
...
def create_dimmed_bee_image(bee_name, width=100, height=100):
    ...
    # Make it semi-transparent and grayish
    pixels = pil_image.load()
    for i in range(pil_image.width):
        for j in range(pil_image.height):
            r, g, b, a = pixels[i, j]
            # Desaturate (make grayscale) and reduce opacity
            gray = int(0.299 * r + 0.587 * g + 0.114 * b)
            pixels[i, j] = (gray, gray, gray, 150)  # 150 alpha for 60% opacity
    
    # Convert back to RGB
    """Entry point for the reorganized Bees Project GUI.

    This file now delegates implementation to `gui.py`. Keep this file as a
    convenient runner for the application.
    """

    import tkinter as tk

    from gui import BeesProjectApp


    def main():
        root = tk.Tk()
        app = BeesProjectApp(root)
        root.mainloop()
